...
def obb2poly_np_data(rbboxes, version='oc'):
    """Convert oriented bounding boxes to polygons.
    ...
    """
    if version == 'oc':
        results = obb2poly_np_data_oc(rbboxes)
    elif version == 'le135':
        results = obb2poly_np_data_le135(rbboxes)
    elif version == 'le90':
        results = obb2poly_np_data_le90(rbboxes)
    else:
        raise NotImplementedError
    return results
...
def obb2poly_np_data_oc(rbboxes):
    """Convert oriented bounding boxes to polygons.
    ...
    polys = get_best_begin_point_data(polys)
    return polys


def obb2poly_np_data_le135(rrects):
    """Convert oriented bounding boxes to polygons.
    ...
    for rrect in rrects:
        ...
        x0, x1, x2, x3 = poly[0, :4] + x_ctr
        y0, y1, y2, y3 = poly[1, :4] + y_ctr
        poly = np.array([x0, y0, x1, y1, x2, y2, x3, y3],
                        dtype=np.float32)
        polys.append(poly)
    polys = np.array(polys)
    polys = get_best_begin_point_data(polys)
...
def obb2poly_np_data_le90(obboxes):
    """Convert oriented bounding boxes to polygons.
...
def get_best_begin_point_single_data(coordinate):
    """Get the best begin point of the single polygon.
...
def get_best_begin_point_data(coordinates):
    """Get the best begin points of polygons.
    ...
    coordinates = list(get_best_begin_point_single_data(coordinates))
    coordinates = np.array(coordinates)
    return coordinates
